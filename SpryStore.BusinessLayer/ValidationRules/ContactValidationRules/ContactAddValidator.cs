...
namespace SpryStore.BusinessLayer.ValidationRules.ContactValidationRules
{
    public class ContactAddValidator:AbstractValidator<Contact>
    {
        public ContactAddValidator()
        {
            ...
            RuleFor(x=>x.NameSurname).MaximumLength(30).WithMessage("Lütfen en fazla 30 karakter girin");
            RuleFor(x=>x.Mail).MaximumLength(50).WithMessage("Lütfen en fazla 50 karakter girin");
            RuleFor(x=>x.MessageContent).MaximumLength(1000).WithMessage("Lütfen daha az karakter girin");
